...
struct TokenRepository: TokenRepositoryProtocol {
    ...
    func save(_ auth: Authorization) -> Observable<Authorization> {
        return Observable.create { emitter in
            ...
            guard keychainManager?.save(key: .authorization, data: data) ?? false || keychainManager?.update(key: .authorization, data: data) ?? false else { emitter.onError(TokenError.saveFail)
                return Disposables.create()
            }
